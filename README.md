...
- Modbus RTU ファンクションコード: `03h`, `06h`, `10h`
  - 03h: 保持レジスタ（16bit）の読み出し
  - 06h: 保持レスタへの書き込み
  - 10h: 複数の保持レスタへの書き込み
- モータ制御に必要なコマンド
  - 回転速度設定（運転データ No.0..7）
    - 0 r/min、80 ～ 3000 r/min
  - 正転(FWD) / 逆転(REV) / 停止(STOP-MODE)
...
## ■ モータドライバの設定（BLVD10KM）
左右のモータドライバを２台チェーン接続する  
通信パラメータ初期値は `8E1`（Even parity, 1 stop bit）です。
- `SW2-No.5 = ON`
  - プロトコル：Modbus
- `SW2-No.1..3 = OFF/OFF/OFF`
  - 通信速度（9600bps）
- `SW2-No.4, No.6 = OFF`
- `SW2-No.7 = ON`
  - 終端抵抗(120Ω)、パスの末端ドライバのみONとする
- 右モータドライバ: `SW3 = 1`, `SW2-No.8 = OFF`（Slave ID=1）
- 左モータドライバ: `SW3 = 2`, `SW2-No.8 = OFF`（Slave ID=2）

## ■ 配線

<img src=docs/rs485_connector.png width=50%><img src=docs/blvd10km_rs485.png width=50%>

- BLVD CN5/CN6 `TR+`(RS-485 通信用信号(+)) -> USB-RS485 `A(+)`
- BLVD CN5/CN6 `TR-`(RS-485 通信用信号(-)) -> USB-RS485 `B(-)`  
...
<img src=docs/propo.png>  

`ch-C`/`ch-D`/`ch-H` のPWM入力を使って左右モータを直接制御します。  
プロポは、[MC-8 2.4GHz MX-F 送受信機セット（MR-8付属）](https://kondo-robot.com/product/10710)を使用します。`ch-C`、`ch-D`を用いて左右のモータを制御します。`ch-H`を用いて、車体速度変更を行います。

- デフォルト最大並進速度: `0.945 km/h`（`0.2625 m/s`）
- デフォルト最大角速度: `1.57 rad/s`
- デフォルトPWM校正値:
  - `CH-C`: min=`945`, center=`1484`, max=`2132`
  - `CH-D`: min=`1114`, center=`1501`, max=`1870`
  - `CH-H`: min=`997`, center=`1484`, max=`1994`
- デフォルトGPIO: `CH-C=24`, `CH-D=4`, `CH-H=14`
